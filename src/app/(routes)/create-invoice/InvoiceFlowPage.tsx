...
import type { CreateSaleResponse } from "../sales-dashboard/service/salesApi";
...
export function InvoiceFlowPage({ editInvoiceId }: { editInvoiceId?: string }) {
 const [taxes, setTaxes] = useState<TaxCategory[]>([]);
 const [loading, setLoading] = useState(true);
 const [error, setError] = useState(false);
 const [selectedId, setSelectedId] = useState<string>("");
 const [taxFromInvoice, setTaxFromInvoice] = useState<string | null>(null);
 ...
 useEffect(() => {
  ...
  invoicesApi
   ...
   .then((res) => {
    ...
    const tid = res.data.taxId;
    if (tid) setTaxFromInvoice(String(tid));
   })
   .catch(() => {});
  return () => {
   cancelled = true;
 ...
 useEffect(() => {
  if (!taxFromInvoice || taxes.length === 0) return;
  if (taxes.some((t) => t._id === taxFromInvoice)) setSelectedId(taxFromInvoice);
 }, [taxFromInvoice, taxes]);

 useEffect(() => {
  if (!selectedId || typeof window === "undefined" || editInvoiceId) return;
  window.localStorage.setItem(SELECTED_TAX_KEY, selectedId);
 }, [selectedId, editInvoiceId]);
 ...
 const writer = useMemo<OrderWriter>(
  () => ({
   ...
   updateSale: editInvoiceId
    ? (id, payload) =>
       invoicesApi.updateInvoice(id, {
        ...payload,
        ...(selectedId ? { taxId: selectedId } : {}),
       }) as Promise<CreateSaleResponse>
    : undefined,
   editInvoiceId,
